    ...
    while (currentDate <= endDate) {
      ...
      await aux.forEach((item, i) => {
        worksheet.addRow({
            ...
            ,cuartoRealOtr: ''
            ,final: `${formattedDate.split('').splice(0,2).join('')}/${formattedDate.split('').splice(2,2).join('')}/${formattedDate.split('').splice(4,4).join('')} ${item.hitos ? item.hitos[item.hitos.length()-2].split(' | ')[0] : ''}`
            ,horaResuelvo: ''
            ,horaMinuta: `${item.hitos ? item.hitos[item.hitos.length()-1].split(' | ')[0] : ''}`
            ,demoraMinuta: ''
