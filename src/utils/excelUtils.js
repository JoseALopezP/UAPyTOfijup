  ...
  while (currentDate <= endDate) {
    ...
    await aux.forEach((item, i) => {
      worksheet.addRow({
        id: `${counter + i}`
        , numeroLeg: `${item.numeroLeg}`
        , fecha: `${String(currentDate.getDate()).padStart(2, '0')}/${String(currentDate.getMonth() + 1).padStart(2, '0')}/${currentDate.getFullYear()}`
        , inicio: item.hitos ? item.hitos[0].split(' | ')[0] : ''
        , fin: item.hitos ? item.hitos[item.hitos.length - 1].split(' | ')[0] : ''
