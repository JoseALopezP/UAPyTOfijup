...
export default function RegistroAudienciaLeft({ item, dateToUse, isHovered, sala, setSala, saeNum, setSaeNum, caratula, setCaratula, razonDemora, setRazonDemora, mpf, setMpf, ufi, setUfi, estado, setEstado, defensa, setDefensa, imputado, setImputado, tipo, setTipo, tipo2, setTipo2, tipo3, setTipo3, partes, setPartes }) {
    ...
    return (
        <form className={`${styles.controlBlockLeft}`} onSubmit={(event) => handleSubmit(event)}>
            ...
            {item.hitos &&
                <span title='Editar Hitos' onClick={() => setShow(!showEditHitos)} className={isHovered ? `${styles.editHitosButtonBlock} ${styles.editHitosButtonBlockHovered}` : `${styles.editHitosButtonBlock}`}><svg className={`${styles.editHitosButtonSVG}`} viewBox="0 0 24 24">
