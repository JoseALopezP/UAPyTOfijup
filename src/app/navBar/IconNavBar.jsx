'use client'
import { useState } from 'react';
import styles from './NavBar.module.css'
import ViewNameText from './ViewNameText';
import Link from 'next/link';
import { usePathname } from 'next/navigation';

export default function IconNavBar({ iconRoute }) {
    const [hovering, setHovering] = useState(false)
    const currentRoute = usePathname()
    return (
        <Link href={'/' + iconRoute} onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}
            className={`${styles.navButton} ${('/' + iconRoute) === currentRoute ? styles.navButtonCurrent : ''}`}>
            {iconRoute === 'tablero' && <svg viewBox="0 0 24 24" stroke={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M3 9.5H21M3 14.5H21M8 4.5V19.5M6.2 19.5H17.8C18.9201 19.5 19.4802 19.5 19.908 19.282C20.2843 19.0903 20.5903 18.7843 20.782 18.408C21 17.9802 21 17.4201 21 16.3V7.7C21 6.5799 21 6.01984 20.782 5.59202C20.5903 5.21569 20.2843 4.90973 19.908 4.71799C19.4802 4.5 18.9201 4.5 17.8 4.5H6.2C5.0799 4.5 4.51984 4.5 4.09202 4.71799C3.71569 4.90973 3.40973 5.21569 3.21799 5.59202C3 6.01984 3 6.57989 3 7.7V16.3C3 17.4201 3 17.9802 3.21799 18.408C3.40973 18.7843 3.71569 19.0903 4.09202 19.282C4.51984 19.5 5.07989 19.5 6.2 19.5Z" strokeWidth="1.5" fill='none' />
            </svg>}
            {iconRoute === '' && <svg viewBox="0 0 28 28" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path fillRule="nonzero" d="M12.592 3.496a2.25 2.25 0 0 1 2.816 0l7.75 6.219A2.25 2.25 0 0 1 24 11.47v11.28A2.25 2.25 0 0 1 21.75 25h-3a2.25 2.25 0 0 1-2.25-2.25v-6a.75.75 0 0 0-.75-.75h-3.5a.75.75 0 0 0-.75.75v6A2.25 2.25 0 0 1 9.25 25h-3A2.25 2.25 0 0 1 4 22.75V11.47c0-.683.31-1.328.842-1.755l7.75-6.219Zm1.877 1.17a.75.75 0 0 0-.938 0l-7.75 6.219a.75.75 0 0 0-.281.585v11.28c0 .414.336.75.75.75h3a.75.75 0 0 0 .75-.75v-6a2.25 2.25 0 0 1 2.25-2.25h3.5A2.25 2.25 0 0 1 18 16.75v6c0 .414.336.75.75.75h3a.75.75 0 0 0 .75-.75V11.47a.75.75 0 0 0-.28-.585l-7.75-6.219Z" />
            </svg>}
            {iconRoute === 'Agregar-Audiencia' && <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="m3.5 6.75.003-2.123A2.25 2.25 0 0 0 2 6.75v10.504a4.75 4.75 0 0 0 4.75 4.75h5.064a6.515 6.515 0 0 1-1.08-1.5H6.75a3.25 3.25 0 0 1-3.25-3.25V6.75Z" />
                <path d="M18 11.174V4.25A2.25 2.25 0 0 0 15.75 2h-9A2.25 2.25 0 0 0 4.5 4.25v13a2.25 2.25 0 0 0 2.25 2.25h3.563a6.475 6.475 0 0 1-.294-1.5H6.75a.75.75 0 0 1-.75-.75v-13a.75.75 0 0 1 .75-.75h9a.75.75 0 0 1 .75.75V11c.516 0 1.018.06 1.5.174Z" />
                <path d="M16.5 12a5.5 5.5 0 1 1 0 11 5.5 5.5 0 0 1 0-11Zm.501 8.503V18h2.496a.5.5 0 0 0 0-1H17v-2.5a.5.5 0 1 0-1 0V17h-2.504a.5.5 0 0 0 0 1H16v2.503a.5.5 0 1 0 1 0Z" />
            </svg>}
            {iconRoute === 'Carga-Juicio' && <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="m3.5 6.75.003-2.123A2.25 2.25 0 0 0 2 6.75v10.504a4.75 4.75 0 0 0 4.75 4.75h5.064a6.515 6.515 0 0 1-1.08-1.5H6.75a3.25 3.25 0 0 1-3.25-3.25V6.75Z" />
                <path d="M18 11.174V4.25A2.25 2.25 0 0 0 15.75 2h-9A2.25 2.25 0 0 0 4.5 4.25v13a2.25 2.25 0 0 0 2.25 2.25h3.563a6.475 6.475 0 0 1-.294-1.5H6.75a.75.75 0 0 1-.75-.75v-13a.75.75 0 0 1 .75-.75h9a.75.75 0 0 1 .75.75V11c.516 0 1.018.06 1.5.174Z" />
                <path d="M16.5 12a5.5 5.5 0 1 1 0 11 5.5 5.5 0 0 1 0-11Zm.501 8.503V18h2.496a.5.5 0 0 0 0-1H17v-2.5a.5.5 0 1 0-1 0V17h-2.504a.5.5 0 0 0 0 1H16v2.503a.5.5 0 1 0 1 0Z" />
            </svg>}
            {iconRoute === 'Registro-Audiencia' && <svg viewBox="0 0 28 28" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M5.25 5.5A3.25 3.25 0 0 0 2 8.75v10.5a3.25 3.25 0 0 0 3.25 3.25h10.502a3.25 3.25 0 0 0 3.25-3.25v-1.58l4.541 3.112a1.25 1.25 0 0 0 1.957-1.03V8.247a1.25 1.25 0 0 0-1.956-1.03l-4.542 3.11V8.75a3.25 3.25 0 0 0-3.25-3.25H5.25Zm13.752 10.352v-3.707L24 8.723v10.554l-4.998-3.425Zm-1.5-7.102v10.5a1.75 1.75 0 0 1-1.75 1.75H5.25a1.75 1.75 0 0 1-1.75-1.75V8.75C3.5 7.784 4.284 7 5.25 7h10.502c.967 0 1.75.784 1.75 1.75Z" />
            </svg>}
            {iconRoute === 'Centro-UGA' && <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M12 2L2 7h20L12 2zm-8 7v10h3V9H4zm6 0v10h4V9h-4zm7 0v10h3V9h-3zM2 20v2h20v-2H2z" />
            </svg>}
            {iconRoute === 'Sorteo-Operador' && <svg viewBox="0 0 256 256" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M216,48V88a8.00018,8.00018,0,0,1-13.65723,5.65674L188,79.314l-31.71582,31.71583a8.00035,8.00035,0,0,1-11.31445-11.314L176.68579,68l-14.343-14.34326A8.00038,8.00038,0,0,1,168,40h40A8.00008,8.00008,0,0,1,216,48Zm-4.93848,112.60889a8.00113,8.00113,0,0,0-8.71875,1.73437L188,176.686,53.65723,42.34326A8.00018,8.00018,0,1,0,42.34277,53.65674L176.686,188l-14.34327,14.34326A8.00038,8.00038,0,0,0,168,216h40a8.00008,8.00008,0,0,0,8-8V168A8.00015,8.00015,0,0,0,211.06152,160.60889ZM99.71582,144.97021l-57.373,57.373a8.00018,8.00018,0,0,0,11.31446,11.31348l57.373-57.37256a8.00035,8.00035,0,0,0-11.31445-11.314Z" />
            </svg>}
            {iconRoute === 'Oficios' && <svg viewBox="2 0 28 28" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M4 5.25A3.25 3.25 0 0 1 7.25 2h13.5A3.25 3.25 0 0 1 24 5.25v12.129c-.12.08-.235.174-.341.28L22.5 18.818V5.25a1.75 1.75 0 0 0-1.75-1.75H7.25A1.75 1.75 0 0 0 5.5 5.25v17.5c0 .966.784 1.75 1.75 1.75h8.068l1.5 1.5H7.25A3.25 3.25 0 0 1 4 22.75V5.25Z" />
                <path d="M10.5 8.75a1.25 1.25 0 1 1-2.5 0 1.25 1.25 0 0 1 2.5 0ZM9.25 15.25a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM9.25 20.5a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM12.75 8a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5ZM12 14a.75.75 0 0 1 .75-.75h6.5a.75.75 0 0 1 0 1.5h-6.5A.75.75 0 0 1 12 14ZM12.75 18.5a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5ZM25.78 19.78l-6 6a.75.75 0 0 1-1.06 0l-2.998-2.998a.75.75 0 0 1 1.06-1.06l2.468 2.467 5.47-5.47a.75.75 0 1 1 1.06 1.061Z" />
            </svg>}
            {iconRoute === 'Situacion-Corporal' && <svg viewBox="0 0 445.969 445.969" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M352.333,294.073c-12.188-17.196-32.426-27.463-54.134-27.463c-21.709,0-41.946,10.267-54.135,27.463
                    c-12.726,17.957-15.237,40.655-7.071,63.915c8.294,23.629,25.382,46.528,41.909,68.672c3.923,5.257,7.628,10.222,11.185,15.156
                    c1.88,2.608,4.898,4.153,8.113,4.153c3.214,0,6.233-1.545,8.112-4.153c3.559-4.938,7.268-9.908,11.194-15.17
                    c16.521-22.14,33.605-45.034,41.9-68.657C367.571,334.728,365.059,312.029,352.333,294.073z M340.535,351.363
                    c-7.282,20.738-23.437,42.387-39.059,63.322c-1.085,1.454-2.181,2.923-3.278,4.397c-1.094-1.469-2.186-2.933-3.268-4.383
                    c-15.626-20.939-31.785-42.592-39.066-63.335c-5.958-16.97-4.354-33.208,4.517-45.725c8.445-11.915,22.582-19.028,37.817-19.028
                    c15.235,0,29.372,7.113,37.817,19.028C344.887,318.155,346.492,334.394,340.535,351.363z M106.019,82.123h15.461
                    c2.379,31,16.052,55.053,38.351,67.952c10.502,6.075,22.278,9.217,34.054,9.217c11.775,0,23.551-3.194,34.053-9.269
                    c22.299-12.898,35.972-36.899,38.351-67.899h15.463c3.983,0,7.586-2.155,9.172-5.808s0.854-7.795-1.866-10.706
                    c-3.648-3.904-7.26-7.925-11.084-12.237c-8.917-10.054-18.32-20.631-28.901-29.73c-12.479-13.533-30.906-21.991-51.101-23.559
                    c-1.123-0.087-6.444-0.111-7.389-0.046c-20.511,1.409-39.255,9.924-51.893,23.638c-10.576,9.096-19.976,19.695-28.89,29.745
                    c-3.826,4.313-7.439,8.388-11.089,12.294c-2.719,2.911-3.452,6.948-1.866,10.602C98.433,79.968,102.036,82.123,106.019,82.123z
                    M217.924,132.762c-14.828,8.578-33.25,8.473-48.079-0.104c-16.077-9.299-26.111-27.535-28.29-50.535h104.658
                    C244.035,105.123,234.001,123.463,217.924,132.762z M193.887,19.469c26.899,1.114,46.663,21.654,65.434,42.654h-130.87
                    C147.221,41.123,166.987,20.583,193.887,19.469z M207.487,374.123c0,5.523-4.477,10-10,10H91.854c-2.97,0-5.786-1.451-7.686-3.734
                    s-2.687-5.423-2.148-8.344L105.5,244.809c8.232-44.442,50.102-64.465,88.154-64.666c0.066-0.001,0.133-0.02,0.2-0.02
                    c0,0,0.008,0,0.013,0h0.009c0.072,0,0.146,0,0.219,0h0.147c0.033,0,0.066,0,0.099,0c17.764,0,34.747,4.264,49.145,12.082
                    c16.129,8.759,28.111,21.749,34.65,37.525c2.115,5.102-0.307,10.96-5.409,13.075c-5.1,2.116-10.952-0.29-13.067-5.393
                    c-9.342-22.537-35.036-37.093-65.509-37.189c-29.904,0.026-62.811,15.028-68.984,48.359l-21.298,115.54h93.619
                    C203.01,364.123,207.487,368.6,207.487,374.123z M298.198,301.941c-15.581,0-28.257,12.676-28.257,28.256
                    s12.676,28.256,28.257,28.256c15.582,0,28.258-12.676,28.258-28.256S313.78,301.941,298.198,301.941z M298.198,338.454
                    c-4.553,0-8.257-3.704-8.257-8.256s3.704-8.256,8.257-8.256c4.554,0,8.258,3.704,8.258,8.256S302.752,338.454,298.198,338.454z"/>
            </svg>}
            {iconRoute === 'Manual' && <svg viewBox="0 0 32 32" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M16 0c-8.836 0-16 7.163-16 16s7.163 16 16 16c8.837 0 16.001-7.163 16.001-16s-7.163-16-16.001-16zM16 30.032c-7.72 0-14-6.312-14-14.032s6.28-14 14-14 14.001 6.28 14.001 14-6.281 14.032-14.001 14.032zM14.53 25.015h2.516v-2.539h-2.516zM15.97 6.985c-1.465 0-2.672 0.395-3.62 1.184s-1.409 2.37-1.386 3.68l0.037 0.073h2.295c0-0.781 0.261-1.904 0.781-2.308s1.152-0.604 1.893-0.604c0.854 0 1.511 0.232 1.971 0.696s0.689 1.127 0.689 1.989c0 0.725-0.17 1.343-0.512 1.855-0.343 0.512-0.916 1.245-1.721 2.198-0.831 0.749-1.344 1.351-1.538 1.806s-0.297 1.274-0.305 2.454h2.405c0-0.74 0.047-1.285 0.14-1.636s0.36-0.744 0.799-1.184c0.945-0.911 1.703-1.802 2.277-2.674 0.573-0.87 0.86-1.831 0.86-2.881 0-1.465-0.443-2.607-1.331-3.424s-2.134-1.226-3.736-1.226z"></path>
            </svg>}
            {iconRoute === 'Pumba' && <svg viewBox="0 0 520 520" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <g><g><g>
                    <path d="M203.862,193.113c-11.336,0-20.56,9.319-20.56,20.827c0,11.505,9.225,20.833,20.56,20.833
                    c11.35,0,20.573-9.328,20.573-20.833C224.435,202.432,215.212,193.113,203.862,193.113z"/>
                    <path d="M307.988,192.951c-11.446,0-20.73,9.397-20.73,20.989s9.284,20.989,20.73,20.989c11.431,0,20.715-9.397,20.715-20.989
                    C328.704,202.347,319.42,192.951,307.988,192.951z"/>
                    <path d="M511.897,128.63c1.105-23.042-6.665-45.143-21.881-62.233c-15.294-17.178-36.266-27.24-59.051-28.332
                    c-28.707-1.391-55.984,12.297-72.685,35.222c-5.252-3.188-10.611-6.158-16.087-8.875C314.63,50.738,285.631,43.804,256,43.804
                    c-29.631,0-58.63,6.934-86.192,20.608c-5.477,2.718-10.838,5.687-16.089,8.875c-16.705-22.925-43.991-36.609-72.683-35.223
                    c-22.785,1.093-43.757,11.155-59.051,28.333C6.769,83.488-1.002,105.588,0.104,128.63c1.588,33.098,21.205,61.678,50.18,74.865
                    c-5.294,15.177-9.097,30.752-11.286,46.432l-19.673-4.568l-7.091,30.535l24.561,5.703c0.057,30.95,6.567,59.408,19.147,84.57
                    l-43.708,10.149l7.091,30.535l54.239-12.595c8.198,10.635,17.721,20.461,28.543,29.383
                    c39.414,32.498,94.068,50.394,153.894,50.394s114.48-17.897,153.894-50.394c10.822-8.923,20.345-18.748,28.543-29.384
                    l54.242,12.596l7.091-30.535l-43.712-10.15c12.58-25.16,19.088-53.618,19.147-84.567l24.565-5.704l-7.091-30.535l-19.677,4.569
                    c-2.189-15.681-5.991-31.256-11.286-46.433C490.691,190.308,510.308,161.728,511.897,128.63z M31.415,127.128
                    c-0.711-14.797,4.256-28.961,13.982-39.886c9.647-10.836,22.838-17.18,37.142-17.868c17.932-0.867,35.007,7.681,45.601,22.024
                    c-9.794,8.012-19.075,16.939-27.785,26.751c-15.182,17.107-27.93,36.178-37.931,56.404
                    C44.506,165.996,32.415,147.98,31.415,127.128z M235.565,336.885c-8.105-8.101-12.345-16.603-12.345-20.814
                    c0-3.757,0.644-4.028,3.037-5.035c6.832-2.873,20.248-3.206,29.745-3.206c9.498,0,22.914,0.332,29.746,3.206
                    c2.394,1.008,3.038,1.279,3.038,5.035c0,4.211-4.241,12.713-12.345,20.814c-7.97,7.965-16.479,12.263-20.438,12.263
                    S243.535,344.85,235.565,336.885z M424.366,358.805l-60.373-14.019l-7.091,30.535l47.134,10.945
                    c-4.355,4.626-9.044,9.031-14.084,13.187c-30.418,25.081-71.971,40.016-118.277,42.765v-64.853
                    c12.3-4.748,22.318-13.706,26.924-18.309c6.474-6.47,21.533-23.614,21.533-42.986c0-15.849-7.894-27.898-22.227-33.929
                    c-9.555-4.021-21.695-5.66-41.902-5.66s-32.348,1.639-41.902,5.66c-14.334,6.03-22.227,18.08-22.227,33.929
                    c0,19.372,15.059,36.515,21.533,42.986c4.605,4.603,14.622,13.561,26.923,18.309v64.853
                    c-46.308-2.748-87.862-17.685-118.281-42.765c-5.04-4.156-9.728-8.561-14.083-13.185l47.137-10.946l-7.091-30.535l-60.377,14.02
                    c-11.693-20.574-18.24-44.108-19.326-69.891l79.703,18.508l7.091-30.535l-85.446-19.841
                    c5.313-42.565,24.39-84.563,54.143-118.088c36.519-41.147,83.468-63.806,132.201-63.806s95.682,22.661,132.201,63.806
                    c29.753,33.524,48.83,75.523,54.143,118.089l-85.442,19.84l7.09,30.534l79.699-18.507
                    C442.605,314.698,436.059,338.231,424.366,358.805z M480.585,127.129c-1,20.852-13.091,38.867-31.009,47.428
                    c-10.001-20.226-22.748-39.298-37.931-56.404c-8.709-9.814-17.991-18.739-27.785-26.751
                    c10.592-14.345,27.664-22.893,45.602-22.025c14.302,0.686,27.493,7.031,37.141,17.868
                    C476.33,98.167,481.296,112.332,480.585,127.129z"/>
                </g></g></g>
            </svg>}
            {iconRoute === 'Listas-Desplegables' && <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path fillRule="nonzero" d="M6.248 16.002c.966 0 1.75.784 1.75 1.75v2.498A1.75 1.75 0 0 1 6.248 22H3.75A1.75 1.75 0 0 1 2 20.25v-2.498c0-.966.784-1.75 1.75-1.75h2.498Zm0 1.5H3.75a.25.25 0 0 0-.25.25v2.498c0 .138.112.25.25.25h2.498a.25.25 0 0 0 .25-.25v-2.498a.25.25 0 0 0-.25-.25Zm3.5.498h11.505a.75.75 0 0 1 .102 1.493l-.102.007H9.748a.75.75 0 0 1-.102-1.493L9.748 18h11.505H9.748Zm-3.5-8.999c.966 0 1.75.784 1.75 1.75v2.498a1.75 1.75 0 0 1-1.75 1.75H3.75A1.75 1.75 0 0 1 2 13.249V10.75c0-.966.784-1.75 1.75-1.75h2.498Zm0 1.5H3.75a.25.25 0 0 0-.25.25v2.498c0 .138.112.25.25.25h2.498a.25.25 0 0 0 .25-.25V10.75a.25.25 0 0 0-.25-.25Zm3.5.499h11.505a.75.75 0 0 1 .102 1.493l-.102.007H9.748a.75.75 0 0 1-.102-1.493L9.748 11h11.505H9.748Zm-3.5-9c.966 0 1.75.784 1.75 1.75v2.498a1.75 1.75 0 0 1-1.75 1.75H3.75A1.75 1.75 0 0 1 2 6.248V3.75C2 2.784 2.784 2 3.75 2h2.498Zm0 1.5H3.75a.25.25 0 0 0-.25.25v2.498c0 .138.112.25.25.25h2.498a.25.25 0 0 0 .25-.25V3.75a.25.25 0 0 0-.25-.25Zm3.5.5h11.505a.75.75 0 0 1 .102 1.493l-.102.007H9.748a.75.75 0 0 1-.102-1.493L9.748 4h11.505H9.748Z" />
            </svg>}
            {iconRoute === 'Administracion-Logistica' && <svg fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} viewBox="0 0 24 24">
                <path d="M21.71,7.38,16.62,2.29a1,1,0,0,0-1.41,0,1,1,0,0,0,0,1.42L17,5.54,11.58,11l-1-1h0a3,3,0,0,0-4.25,0L2.88,13.42A3,3,0,0,0,2,15.55V19a3,3,0,0,0,3,3H8.45a3,3,0,0,0,2.13-.88L14,17.69a3,3,0,0,0,0-4.25l-1-1L18.46,7l1.83,1.83a1,1,0,0,0,1.42,0A1,1,0,0,0,21.71,7.38ZM12.6,16.27,9.16,19.71a1,1,0,0,1-.71.29H5a1,1,0,0,1-1-1V15.55a1,1,0,0,1,.29-.71L7.73,11.4a1,1,0,0,1,1.41,0l1,1-.89.9a1,1,0,0,0,0,1.41A1,1,0,0,0,10,15a1,1,0,0,0,.7-.29l.9-.89,1,1A1,1,0,0,1,12.6,16.27Z" />
            </svg>}
            {iconRoute === 'audienciasUAC/control' && <svg stroke={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} viewBox="0 0 24 24">
                <path d="M11 6L21 6.00072M11 12L21 12.0007M11 18L21 18.0007M3 11.9444L4.53846 13.5L8 10M3 5.94444L4.53846 7.5L8 4M4.5 18H4.51M5 18C5 18.2761 4.77614 18.5 4.5 18.5C4.22386 18.5 4 18.2761 4 18C4 17.7239 4.22386 17.5 4.5 17.5C4.77614 17.5 5 17.7239 5 18Z" strokeWidth="1.5" />
            </svg>}
            {iconRoute === 'Control-UAC' && <svg stroke={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} fill="none" strokeWidth="1.5" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>}
            {iconRoute === 'Minuta-Juicio' && (
                <svg stroke={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} viewBox="0 0 110.57153 112.57477">
                    <g transform="translate(-206.0073,-31.50867)">
                        <path
                            d="m 241.25563,143.81701 c -1.03176,-0.41571 -1.14188,-1.05963 -1.13919,-6.66149 0.002,-5.11857 0.033,-5.44044 0.63878,-6.74158 1.12166,-2.40904 3.39408,-4.20402 5.7166,-4.51554 l 0.9913,-0.13296 0.11454,-2.54881 c 0.0871,-1.93877 0.25849,-2.84748 0.71595,-3.79668 0.9497,-1.97057 2.49627,-3.52914 4.46246,-4.4971 l 1.76645,-0.86961 h 23.81806 23.81806 l 1.50155,0.7039 c 1.99971,0.93742 3.75202,2.59926 4.62741,4.38848 0.56766,1.16027 0.74677,1.98343 0.86531,3.97696 l 0.14948,2.51386 1.43368,0.36595 c 2.54639,0.64999 4.03628,1.92169 5.21938,4.45501 0.58595,1.2547 0.61795,1.61187 0.6204,6.92611 l 0.003,5.60319 -0.69714,0.54837 -0.69714,0.54837 -36.67525,-0.0168 c -20.46343,-0.009 -36.93076,-0.11971 -37.25327,-0.24966 z m 71.08947,-7.47676 c 0,-4.01718 -0.3234,-5.02092 -1.87452,-5.81804 -0.8489,-0.43624 -2.81355,-0.46027 -32.45079,-0.39687 l -31.54719,0.0675 -0.99656,0.99584 -0.99656,0.99585 -0.0875,3.45397 c -0.0481,1.89968 -0.0221,3.62444 0.0578,3.83279 0.1251,0.326 4.8703,0.37884 34.02034,0.37884 h 33.87497 z m -7.41414,-12.29899 c -0.0103,-3.14962 -1.39283,-5.03918 -4.11416,-5.6228 -0.88622,-0.19006 -8.46063,-0.25739 -23.26937,-0.20684 l -21.95545,0.075 -1.23375,0.72526 c -1.64683,0.9681 -2.3676,2.19007 -2.53707,4.30129 -0.0744,0.92741 -0.0639,1.87247 0.0235,2.10014 0.14264,0.37171 2.85881,0.41393 26.62548,0.41393 h 26.46664 z m -91.54002,-1.25094 c -2.0981,-0.6766 -4.02145,-2.02669 -5.30108,-3.72108 -1.63989,-2.17141 -2.18453,-4.00137 -2.06725,-6.94574 0.0789,-1.98027 0.21965,-2.57549 0.96877,-4.09623 0.94465,-1.91765 1.87506,-2.82553 27.83081,-27.156666 5.02047,-4.706231 9.60232,-9.017534 10.1819,-9.580674 l 1.05377,-1.02389 -2.47466,-2.485 -2.47467,-2.485 -1.48952,0.696407 c -2.2439,1.049106 -4.62931,0.977331 -6.8418,-0.205862 -2.69713,-1.442367 -4.02665,-3.673601 -4.03204,-6.766674 -0.006,-3.184392 0.0339,-3.234721 12.08131,-15.403129 12.12792,-12.249762 11.93511,-12.098431 15.42696,-12.108063 2.29136,-0.0063 3.8076,0.593565 5.39371,2.133971 1.64588,1.598462 2.13448,2.830998 2.14111,5.401181 0.005,1.872516 -0.10275,2.461428 -0.64269,3.51831 l -0.6485,1.269352 11.37701,11.377005 11.377,11.377005 1.44232,-0.671377 c 2.92209,-1.360183 6.58141,-0.710462 8.78652,1.560068 1.47201,1.515682 2.12303,3.130545 2.13827,5.30401 0.0233,3.323407 0.1038,3.220605 -12.05446,15.396797 -9.64769,9.661912 -11.06212,10.979342 -12.46255,11.607818 -2.13258,0.957041 -4.67725,0.844611 -6.77941,-0.299523 -3.52727,-1.919781 -4.94992,-6.493339 -3.1507,-10.128903 l 0.66069,-1.335008 -2.40507,-2.39507 c -1.32279,-1.317289 -2.48347,-2.39507 -2.57929,-2.39507 -0.0958,0 -7.64356,7.947421 -16.77275,17.660935 -9.1292,9.71351 -17.28123,18.32573 -18.11563,19.13825 -2.22108,2.16286 -4.03189,2.9551 -6.9962,3.06085 -1.57386,0.0561 -2.78495,-0.0452 -3.57188,-0.299 z m 6.06362,-4.67742 c 0.56618,-0.34105 8.94214,-9.02542 18.61323,-19.298603 l 17.58381,-18.678528 -3.23729,-3.414179 -3.23729,-3.414179 -1.42273,1.325712 c -0.7825,0.729142 -9.30027,8.741971 -18.92837,17.806287 -13.72126,12.91779 -17.62501,16.73451 -18.05781,17.65525 -1.52038,3.23446 0.17049,7.20703 3.61448,8.49199 1.40235,0.52322 3.78656,0.30052 5.07197,-0.47375 z m 51.79676,-22.195902 c 0.84285,-0.313306 21.07826,-20.369179 21.66418,-21.471969 0.8233,-1.549568 0.20948,-3.801748 -1.25811,-4.616132 -0.77525,-0.430198 -2.56634,-0.438277 -3.25708,-0.01469 -0.29104,0.178476 -5.24927,5.085585 -11.01828,10.904686 -7.918,7.98675 -10.5311,10.779697 -10.66041,11.394115 -0.30412,1.445098 0.60394,3.049614 2.09952,3.709779 0.97692,0.431223 1.47193,0.450413 2.43018,0.09421 z m 3.32596,-18.708401 7.60479,-7.608189 -11.37654,-11.374232 -11.37654,-11.374233 -7.67348,7.670058 -7.67347,7.670059 11.31006,11.312362 c 6.22054,6.2218 11.37089,11.312363 11.44523,11.312363 0.0743,0 3.55731,-3.423685 7.73995,-7.608188 z M 248.51332,51.694802 c 5.73127,-5.721085 10.60438,-10.748772 10.82915,-11.172638 0.88285,-1.664853 -0.26098,-4.116215 -2.15305,-4.614264 -1.91191,-0.50327 -1.98003,-0.44762 -13.2783,10.848075 -7.58204,7.580304 -10.70717,10.875527 -10.82562,11.414835 -0.39517,1.799217 0.46862,3.435559 2.11176,4.000443 2.11753,0.727973 1.99559,0.823911 13.31606,-10.476451 z"
                            style={{ fill: 'var(--nav-icon-inactive)' }}
                        />
                    </g>
                </svg>
            )}
            {iconRoute === 'Abogados' && <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
            </svg>}
            {iconRoute === 'Solicitudes-Audiencia' && (

                <svg stroke={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')} fill="none" viewBox="0 0 24 24">
                    <path d="M12,14a1,1,0,1,0-1-1A1,1,0,0,0,12,14Zm5,0a1,1,0,1,0-1-1A1,1,0,0,0,17,14Zm-5,4a1,1,0,1,0-1-1A1,1,0,0,0,12,18Zm5,0a1,1,0,1,0-1-1A1,1,0,0,0,17,18ZM7,14a1,1,0,1,0-1-1A1,1,0,0,0,7,14ZM19,4H18V3a1,1,0,0,0-2,0V4H8V3A1,1,0,0,0,6,3V4H5A3,3,0,0,0,2,7V19a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V7A3,3,0,0,0,19,4Zm1,15a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V10H20ZM20,8H4V7A1,1,0,0,1,5,6H19a1,1,0,0,1,1,1ZM7,18a1,1,0,1,0-1-1A1,1,0,0,0,7,18Z" />
                </svg>
            )}
            {iconRoute === 'Extractor-Anuladas' && (
                <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                    <path d="M12 3C7.03 3 3 7.03 3 12s4.03 9 9 9 9-4.03 9-9-4.03-9-9-9zm0 16c-3.86 0-7-3.14-7-7s3.14-7 7-7 7 3.14 7 7-3.14 7-7 7zm-1-11h2v6h-2zm0 8h2v2h-2z"/>
                </svg>
            )}
            {iconRoute === 'Notificaciones' && (
                <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                    <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-2 1H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5v6z"/>
                </svg>
            )}
            {iconRoute === 'Notificaciones-Traslados' && (
                <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                    <path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/>
                </svg>
            )}
            {iconRoute === 'Traducciones-Notificaciones' && (
                <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                    <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-2 1H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5v6z" transform="translate(-1.5, -2) scale(0.85)" />
                    <path d="M19.43 12.98c.04-.32.07-.64.07-.98s-.03-.66-.07-.98l2.11-1.65c.19-.15.24-.42.12-.64l-2-3.46c-.12-.22-.39-.3-.61-.22l-2.49 1c-.52-.4-1.08-.73-1.69-.98l-.38-2.65C14.46 2.18 14.25 2 14 2h-4c-.25 0-.46.18-.49.42l-.38 2.65c-.61.25-1.17.59-1.69.98l-2.49-1c-.23-.09-.49 0-.61.22l-2 3.46c-.13.22-.07.49.12.64l2.11 1.65c-.04.32-.07.65-.07.98s.03.66.07.98l-2.11 1.65c-.19.15-.24.42-.12.64l2 3.46c.12.22.39.3.61.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.03.24.24.42.49.42h4c.25 0 .46-.18.49-.42l.38-2.65c.61-.25 1.17-.59 1.69-.98l2.49 1c.23.09.49 0 .61-.22l2-3.46c.12-.22.07-.49-.12-.64l-2.11-1.65zM12 15.5c-1.93 0-3.5-1.57-3.5-3.5s1.57-3.5 3.5-3.5 3.5 1.57 3.5 3.5-1.57 3.5-3.5 3.5z" transform="translate(10, 10) scale(0.55)" />
                </svg>
            )}
            {iconRoute === 'Gestion-Usuarios' && (
                <svg viewBox="0 0 24 24" fill={('/' + iconRoute) === currentRoute ? 'var(--nav-icon-active)' : (hovering ? 'var(--nav-icon-hover)' : 'var(--nav-icon-inactive)')}>
                    <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4zm5-2c1.66 0 3-1.34 3-3s-1.34-3-3-3c-.36 0-.7.09-1 .24a3.98 3.98 0 0 1 0 5.52c.3.15.64.24 1 .24zm2.5 2.1c1.23.8 2.5 1.9 2.5 3.4v2h3v-2c0-2.1-3.6-3.2-5.5-3.4z"/>
                </svg>
            )}
            {hovering && <ViewNameText viewName={iconRoute} />}
        </Link>
    );
}