...
export default function SitCorporalIndiv({aud, date, selected, selectedF}) {
    ...
    const selectionKey = (aud?.numeroLeg || '') + (aud?.hora || '')
    return (
        <div className={aud?.situacion ? `${styles.sitCorporalIndivBlock} ${styles.sitCorporalIndivBlockFilled}` : `${styles.sitCorporalIndivBlock}`}>
        <div className={`${styles.sitCorporalIndivFirst}`} onClick={() => selectedF((selected !== selectionKey) && selectionKey)}>
        ...
        <span className={(selectionKey === selected) ? `${styles.sitCorporalIndivSecond}` : `${styles.sitCorporalIndivSecondHidden}`}>
