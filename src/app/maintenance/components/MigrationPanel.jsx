...
import firebase_app from '@/firebase new/config'; // Usamos la configuración de 'firebase new'

const db = getFirestore(firebase_app);

// Manejo seguro de fechas para evitar problemas de Timezone
...
const MigrationPanel = () => {
    ...
    const handleMigration = async () => {
        ...
        for (const dateObj of datesToProcess) {
            ...
            try {
                // 1. Validar si ya existe en el esquema nuevo
                // Comprobamos si el documento existe y si tiene propiedades (keys)
                ...
                // 2. Obtener datos del esquema viejo
                ...
                // Set usado para garantizar que no haya colisiones de ID mientras iteramos
                const seenIds = new Set();

                for (const docSnap of oldSnap.docs) {
                    const data = docSnap.data();

                    // Identificar si tiene un ID
                    let uniqueId = data.id || data.aId;

                    // Si no hay id único o ya fue visto (duplicado), generar uno nuevo con documento vacío
                    ...
                    // El ID debe ser la clave ancla para ambos esquemas nuevo
                    const dataWithId = { ...data, id: uniqueId };
                    
                    // Llenar objeto para audienciasView (simulando dot notation con merge true post batch)
                    viewObj[uniqueId] = dataWithId;

                    // Referencia del nuevo documento en audiencias
                    const newDocRef = doc(db, 'audiencias', dateStr, 'audiencias', uniqueId);
                    batch.set(newDocRef, dataWithId);

                    // Limpieza Post-Migración:
                    // Borrar el archivo viejo si su doc.id no es igual a nuestro uniqueId procesado
                    // Ej. si el docSnap.id era un formato "hora+legajo"
                ...
                if (processedCount > 0) {
                    // Actualizar el documento espejo en audienciasView atómicamente
                ...
                break; // Detenemos script para no corromper datos subsiguientes
            }
        }
