...
export default function TableRow({ audData, dateToUse, autofillB, index, onStatusChange, forceSave }) {
    ...
    const autoFill = () => {
        ...
        demora === '' && (savedData.demora !== undefined && savedData.demora !== '' ? setDemora(savedData.demora) : audData.inicioReal && audData.inicioProgramada && String(audData.inicioReal).includes(':') && String(audData.inicioProgramada).includes(':') && setDemora((parseInt(audData.inicioReal.split(':')[0]) * 60 + parseInt(audData.inicioReal.split(':')[1])) - (parseInt(audData.inicioProgramada.split(':')[0]) * 60 + parseInt(audData.inicioProgramada.split(':')[1]))))
        duracionProgramada === '' && (savedData.duracionProgramada !== undefined && savedData.duracionProgramada !== '' ? setDuracionProgramada(savedData.duracionProgramada) : audData.inicioProgramada && audData.finProgramada && String(audData.inicioProgramada).includes(':') && String(audData.finProgramada).includes(':') && setDuracionProgramada((parseInt(audData.finProgramada.split(':')[0]) * 60 + parseInt(audData.finProgramada.split(':')[1])) - (parseInt(audData.inicioProgramada.split(':')[0]) * 60 + parseInt(audData.inicioProgramada.split(':')[1]))))
        durReal === '' && (savedData.durReal !== undefined && savedData.durReal !== '' ? setDurReal(savedData.durReal) : audData.finReal && audData.inicioReal && String(audData.finReal).includes(':') && String(audData.inicioReal).includes(':') && setDurReal(parseInt(audData.finReal.split(':')[0]) * 60 + parseInt(audData.finReal.split(':')[1]) - (parseInt(audData.inicioReal.split(':')[0]) * 60 + parseInt(audData.inicioReal.split(':')[1]))))
    ...
    return (
        <><tr className={completado ? `${styles.tableRow} ${styles.tableRowCompleted}` : `${styles.tableRow}`} key={index}>
            ...
            <td className={`${styles.cellBodyFixed} ${styles.cellBodyTablero}`}>
                {tabItem.hitos && tabItem.hitos.length > 0 && tabItem.hora && String(tabItem.hora).includes(':') && String(tabItem.hitos[0]).split(' | ')[0].includes(':') && (parseInt((tabItem.hitos[0].split(' | ')[0].split(':')[0]) * 60 + parseInt(tabItem.hitos[0].split(' | ')[0].split(':')[1])) - (parseInt(tabItem.hora.split(':')[0]) * 60 + parseInt(tabItem.hora.split(':')[1])))}</td>
