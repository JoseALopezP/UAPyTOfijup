...
export function AddAudienciaIndiv({date, element}) {
    ...
    const [minuto, setMinuto] = useState(String(element?.hora || '').split(':')[1] || '')
    ...
    const [horaBis, setHoraBis] = useState(String(element?.hora || '').split(':')[0] || '')
    const [minutoBis, setMinutoBis] = useState(String(element?.hora || '').split(':')[1] || '')
