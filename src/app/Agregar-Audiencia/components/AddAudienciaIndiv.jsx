    ...
    return(
        <form onSubmit={(event) => handleSubmit(event)} className={`${styles.tableRow}`}>
            ...
            <span className={`${styles.tableCell} ${styles.tableCellJuez}`}>
                <button className={`${styles.colButton}`} onClick={toggleJuezFormat}>{juez.includes('+') ? 
                    <><svg viewBox="0 0 26 26">
                        <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd"><g fill="#212121" fill-rule="nonzero">
                ...
                {juez.includes('+') && <><input type='string' className={`${styles.tableCellInput} ${styles.tableCellJuezInput}`} title={juez2} value={juez2} onChange={e => handleJuezChange(e.target.value, 1)}/>
