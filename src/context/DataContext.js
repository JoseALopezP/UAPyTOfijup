...
import deleteDocumentAndObject from "@/firebase/firestore/deleteDocumentAndObject.js";
import { updateDocumentAndObjectField } from "@/firebase/firestore/updateDocumentAndObjectField.js";
import addStringToArray from "@/firebase/firestore/addStringToArray.js";
import removeStringFromArray from "@/firebase/firestore/removeStringFromArray.js";
import { removeObject } from "@/firebase/firestore/removeObject.js";
import { addOrUpdateObject } from "@/firebase/firestore/addOrUpdateObject.js";
import { countDocs } from "@/firebase/firestore/countDocs.js";
import { pushItemToDocumentAndObjectField } from "@/firebase/firestore/pushItemToDocumentAndObjectField.js";
import updateRealTimeFunction from "@/firebase/firestore/updateRealTimeFunction.js";
import { updateDocumentField } from "@/firebase/firestore/updateDocumentField.js";
import addObjectToDocument from "@/firebase/firestore/addObjectToDocument.js";
import { yearFunction } from "@/utils/dateUtils.js";
import { updateInternalFieldJuicio } from "@/firebase/firestore/updateInternalFieldJuicio.js";
import replaceDocument from "@/firebase/firestore/replaceDocument.js";
import { updateInternalUALData } from "@/firebase/firestore/updateInternalUALData.js";
import { updateJuicioBlockStatus } from "@/firebase/firestore/updateJuicioBlockStatus.js";
import { updateDocumentOnly } from "@/firebase/firestore/updateDocumentOnly.js";
...
export const DataContextProvider = ({ defaultValue = [], children }) => {
    ...
    const addAudiencia = async (data, date, customId = null) => {
        try {
            const audId = await addOrUpdateDocument("audiencias", date, 'audiencias', data, customId);
            if (data.resuelvo && data.resuelvo !== "") {
                await updateLegajosDatabase({ ...data, id: audId });
            }
        } catch (error) {
    ...
    const syncLegajo = async (date, audId, property, newValue) => {
        try {
            const audItem = bydate.find(a => a.id === audId);
            let legajoNum = audItem?.numeroLeg;
            let currentResuelvo = audItem?.resuelvo;

            if (!legajoNum) {
                const docData = await getDocument('audiencias/' + date + '/audiencias', audId);
                legajoNum = docData?.numeroLeg;
                currentResuelvo = docData?.resuelvo;
            }

            if (legajoNum) {
                const isNowSettingResuelvo = property === 'resuelvo' && newValue && newValue !== "";
                const alreadyHadResuelvo = currentResuelvo && currentResuelvo !== "";

                if (isNowSettingResuelvo || alreadyHadResuelvo) {
                    if (isNowSettingResuelvo && !alreadyHadResuelvo) {
                        const fullDoc = await getDocument('audiencias/' + date + '/audiencias', audId);
                        await updateLegajosDatabase({ ...fullDoc, [property]: newValue });
                    } else {
                        await updateLegajosDatabase({ numeroLeg: legajoNum, id: audId, [property]: newValue });
                    }
                }
            }
        } catch (e) {
            console.error("Error syncing to legajos:", e);
        }
    }
    ...
    const updateData = async (date, audId, property, newValue) => {
        try {
            ...
            await syncLegajo(date, audId, property, newValue)
        } catch (error) {
            setErrorMessage(`${error.message}`);
        }
    }
